 ...
 */
class JpaLazyAsNullCondition<S, D> implements Condition<S, D> {
    ...
    private void setNullForAllLazyLoadEntities(Object source, PersistenceUnitUtil unitUtil) {
        for (Field field : source.getClass().getDeclaredFields()) {
            ...
            try {
                ...
                if (fieldValue != null) {
                    boolean isEntity = Arrays.stream(field.getType().getAnnotations()).anyMatch(x -> x.annotationType().equals(Entity.class));
